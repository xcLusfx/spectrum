...
export const calcEnvironment = () => {
  ...
  function buildConfigURL(environment) {
    ...
    const configURL = new URL(`${window.location.origin}/aemheadlessconfig.json`);
  ...
  export const getConfigValue = async (configParam, environment) => {
    ...
    const configObject = configElements[0];
    const configValue = configObject ? configObject[configParam] : undefined;
